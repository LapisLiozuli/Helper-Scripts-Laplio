...
Date created: Thu May 20 12:26:43 2021
@author: Julio Hong
Purpose: Convert the string of a list of numbers into a list of strings based on Minecraft Biome dict.
Steps: 
"""
...
def string_to_list(stringed_list):
    # Split the list
    num_list = stringed_list.split(', ')
    # Convert to ints
    num_list = [int(x) for x in num_list]
    
    # Get the biome ID for each num
    biome_list = [biome_dict[num] for num in num_list]
    
    # Convert the list into a string
    # Initialise an empty string
    config_string = ""
    for biome_id in biome_list:
#        config_string.join(biome_id)
        config_string += biome_id + ', '
